...
function xquiz_qcm_menu_show() {
	...
	$sql = "SELECT f.id_form, m.indent, m.margintop, m.marginbottom, m.itemurl, m.bold FROM " . $xoopsDB->prefix("forms_id") . " f LEFT JOIN " . $xoopsDB->prefix("forms_menu") . " m on f.id_form = m.menuid WHERE f.qcm = 1 and m.status =1 ORDER BY m.position";
