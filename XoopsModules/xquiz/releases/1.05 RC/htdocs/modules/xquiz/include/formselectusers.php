 ...
 * @access 		public
 */
class XQuizFormSelectUsers extends XoopsFormElement
{
    /**
     * Options
     *
     ...
     */
    function XQuizFormSelectUsers($caption, $name, $value = null, $size = 1, $multiple = false, $id_form=0, $none = false)
    {
        ...
        if ($none==true)
        	$this->addOption(0, _MI_XQUIZ_NONE);
        
        $forms_users_handler = xoops_getmodulehandler('forms_users', 'xquiz');
        if ($id_form!=0)
        	$criteria = new Criteria('id_form', $id_form);
        else 
        	$criteria = new Criteria('1','1');
        $criteria->setSort('`name`, `email`');
                
		foreach($forms_users_handler->getObjects($criteria, true) as $user_id => $item) {
			$title = $item->getVar('name');
			if (empty($title))
				$title = $item->getVar('email');
			else 
				$title = ' ('.$item->getVar('email').')';
			$this->addOption($user_id, $title);
		}
    }
